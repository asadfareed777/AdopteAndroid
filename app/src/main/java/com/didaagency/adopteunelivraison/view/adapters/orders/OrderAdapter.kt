...
class OrderAdapter(val arrListData: List<Orders>?, private val mContext: Context) :
    RecyclerView.Adapter<OrderAdapter.MyViewHolder>() {

    var onItemClickListner: RvItemClickListner<Orders>? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val binding: ItemViewOrdersLayoutBinding = ItemViewOrdersLayoutBinding.inflate(
            LayoutInflater.from(
                mContext
            ), parent, false
        )
    ...
    inner class MyViewHolder(itemView: ViewBinding) :
        ...
        val binding: ItemViewOrdersLayoutBinding
        init {
            binding = itemView as ItemViewOrdersLayoutBinding
        }
